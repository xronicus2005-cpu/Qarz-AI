const { Debtor, Transaction, sequelize, User } = require('../models/index');
...
// 5. Statistika (ENG MUHIMI - SHU BORMI TEKSHIRING)
exports.getStats = async (req, res) => {
    try {
        const userId = req.user.id;
        const user = await User.findByPk(userId);

        const totalCustomers = await Debtor.count({ where: { UserId: userId } });
        const totalDebtData = await Debtor.sum('total_debt', { where: { UserId: userId } });

        res.json({
            total_customers: totalCustomers,
            total_debt: totalDebtData || 0,
            subscription_end: user ? user.subscription_end_date : null,
